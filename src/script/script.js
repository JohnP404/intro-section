...
const resizeObserver = new ResizeObserver(entry => {
   if (entry[0].contentRect.width >= 960) {
      heroImg.setAttribute('src', '/image-hero-desktop.d267715a.png')
   } else {
      heroImg.setAttribute('src', '/image-hero-mobile.40110e5d.png')
   }
})
...
const expandMenu = function (el) {
   ...
   const nextSibling = el.target.closest('div')
      .nextElementSibling;

   const condition = el.target.closest('div')
      .classList
      .contains('expand-menu--link');
